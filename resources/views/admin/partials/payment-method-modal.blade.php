...
@php($closeUrl = route('admin.payment-methods.index'))
...
<div class="fixed inset-0 z-50 flex items-end justify-center bg-black/50 p-4 sm:items-center">
    <div class="max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-xl bg-white p-5 shadow-xl">
        ...
        <form method="POST" action="{{ $isEdit ? route('admin.payment-methods.update', $method) : route('admin.payment-methods.store') }}" class="space-y-4">
            @csrf
            @if ($isEdit)
                @method('PATCH')
            @endif
            ...
            <div>
                <label class="mb-1 block text-sm font-medium">{{ __('admin.methods.subtitle_field') }}</label>
                <input name="subtitle" value="{{ old('subtitle', $config['subtitle'] ?? '') }}" class="w-full rounded-lg border-slate-300 text-sm">
            </div>

            <div>
                <label class="mb-1 block text-sm font-medium">{{ __('admin.methods.icon') }}</label>
                <input name="icon" value="{{ old('icon', $config['icon'] ?? '') }}" placeholder="wallet, card, ..." class="w-full rounded-lg border-slate-300 text-sm">
            </div>
