...
@section('title', __('member.settings.bind_login_title'))
...
@section('content')
    ...
    >
        <header class="sticky top-14 z-10 flex items-center justify-center border-b border-gray-100 bg-white px-4 py-3">
            ...
            <h1 class="text-base font-semibold text-gray-900">{{ __('member.settings.bind_login_title') }}</h1>
        </header>
            ...
            >
                ...
                <div class="bg-white">
                    <div class="border-b border-gray-100 px-4 py-4">
                        <label for="bind_phone" class="mb-2 block text-sm font-medium text-gray-900">{{ __('member.settings.phone_label') }}</label>
                        <input
                            id="bind_phone"
                            name="phone"
                            type="tel"
                    ...
                    <div class="border-b border-gray-100 px-4 py-4">
                        <label for="bind_phone_password" class="mb-2 block text-sm font-medium text-gray-900">{{ __('member.profile.current_password') }}</label>
                        <input
                            id="bind_phone_password"
        ...
        @if ($showEmailTab)
            ...
            >
                ...
                <div class="bg-white">
                    <div class="border-b border-gray-100 px-4 py-4">
                        <label for="bind_email" class="mb-2 block text-sm font-medium text-gray-900">{{ __('member.settings.email_label') }}</label>
                        <input
                            id="bind_email"
                            name="email"
                            type="email"
                    ...
                    <div class="border-b border-gray-100 px-4 py-4">
                        <label for="bind_email_password" class="mb-2 block text-sm font-medium text-gray-900">{{ __('member.profile.current_password') }}</label>
                        <input
                            id="bind_email_password"
                            name="current_password"
                            type="password"
                            placeholder="{{ __('member.profile.current_password_placeholder') }}"
                            class="portal-plain-input"
        ...
        @if (! $showPhoneTab && ! $showEmailTab)
            <p class="px-4 py-8 text-center text-sm text-gray-500">{{ __('member.settings.bind_login_unavailable') }}</p>
        @endif
    </div>
@endsection
